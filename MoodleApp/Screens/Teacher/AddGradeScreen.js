...
const onDateChange = (date, type) => {
   ...
   const  add_Course = () => {
        ...
        db.transaction(function (tx) {
          tx.executeSql(
            'INSERT INTO table_Grade (g_title, g_date , g_upload , co_id) VALUES (?,?,?,?,?,?)',
            [gradetitle,date,gradefile,value],
            (tx, results) => {
              console.log('Results', results.rowsAffected);
              if (results.rowsAffected > 0) {
                Alert.alert(
                  'Success',
                  'grade uploaded succesfully',
                  [
                    {
                      text: 'Ok',
                  ...
                  db.transaction((tx) => {
                    tx.executeSql('SELECT * FROM table_Grade', [], (tx, results) => {
                      var temp = [];
    ...
    <View style={{ backgroundColor: 'white', flexDirection: 'row' ,marginTop:10}}>
                     ...
                     <Text style={{ color: '#000080', marginTop: 10, marginBottom: 0, fontSize: 20, fontWeight: 'bold',textAlign:'center' }}>Add Grade</Text>
                </View>
          <View style={{ flex: 5, backgroundColor: "#F0B27A" }}>
    
            <View style={{ flex: 5, backgroundColor: "white", borderBottomLeftRadius: 100,  }}>
            <TextInput placeholder="Grade Title" placeholderColor="Black" style={styles.textinput} onChangeText={(gradetitle) => setquiztitle(gradetitle)}  /> 
                <DropDownPicker
                ...
                <TextInput placeholder="Grade File" placeholderColor="Black" style={styles.textinput} onChangeText={(gradefile) => setgradefile(gradefile)} maxLength={225}
                numberOfLines={5}
                multiline={true} />
               <DatePicker
      ...
      />
              <Button mode="contained" onPress={add_Course} style={styles.registerbutton}>Add Grade</Button>
