...
const QuizScreen = ({navigation}) => {
  ...
  const get=() => {
    ...
    db.transaction((tx) => {
      tx.executeSql('SELECT * FROM table_Grade where co_id =? ', [coid.co_id], (tx, results) => {
        var temp = [];
      ...
      let listItemView = (item) => {
        return (
          <View
            ...
            <Text style={styles.text}>Name: {item.g_title}</Text>
            <Text style={styles.text}>last Submission Date: {item.g_date}</Text>            
            <Button mode="contained"  style={styles.stylebutton} onPress={() => Linking.openURL(item.g_upload)}>View Grade</Button>
